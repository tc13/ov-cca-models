...
//Weibull distributed latent and induction periods
...
parameters {
    //Weibull params
    real<lower=0> induction_scale;
    real<lower=0> induction_shape;
    real<lower=0> latent_scale;
    real<lower=0> latent_shape;
}
...
transformed parameters {
    ...
    latent_lp[1] = weibull_lcdf(1 | latent_scale, latent_shape);
    induction_lp[1] = weibull_lcdf(1 | induction_scale, induction_shape);
    
    for(a in 2:(max_age-1)){
      latent_lp[a] = log_diff_exp(weibull_lcdf(a | latent_scale, latent_shape), 
                                   weibull_lcdf(a-1 | latent_scale, latent_shape));
                                  
      induction_lp[a] = log_diff_exp(weibull_lcdf(a | induction_scale, induction_shape), 
                                   weibull_lcdf(a-1 | induction_scale, induction_shape));
    }
...
model {
  ...
  //priors
    induction_scale ~ gamma(3, 1);
    induction_shape ~ gamma(28, 1);
    
    latent_scale ~ gamma(3, 1);
    latent_shape ~ gamma(30, 1);
}
